...
class Censure
{
		...
		$charset = 'UTF-8')
	{
		...
		$s       = PHPUTF8::convert_from($s,       $charset);
		$replace = PHPUTF8::convert_from($replace, $charset);
		...
		if ($is_html)
		{
			...
			$s = PHPUTF8::html_entity_decode($s, $is_htmlspecialchars = true);
		}
		...
		if (strtoupper(substr($charset, 0, 3)) === 'UTF')  #UTF-8, UTF-16, UTF-32
		{
			...
			$s = PHPUTF8::diactrical_remove($s, $additional_chars);
		}
		...
		if ($replace === null || version_compare(PHP_VERSION, '5.2.0', '<'))
		{
			...
			if ($result && $replace === null)
			{
				...
				return PHPUTF8::convert_to($fragment, $charset);
			}
			return false;
		}
		...
		return PHPUTF8::convert_to($ss, $charset);
	}
