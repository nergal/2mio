...
//LogMaster::log("###".var_export($this->request,true));
        return $this->render();
    }
    ...
    */
    protected function parse_request(){
        ...
        //detect edit mode
        if ($this->checkGetParam('editing')) //isset($_GET["editing"])
            $this->editing=true;
        ...
        if ($this->checkGetParam('dhx_sort')) //isset($_GET["dhx_sort"])
             foreach($_GET["dhx_sort"] as $k => $v){
                $k = $this->safe_field_name($k);
                $this->request->set_sort($this->resolve_parameter($k),$v);
            }
        if ($this->checkGetParam('dhx_filter')) //isset($_GET["dhx_filter"])
   ...
   /**
   * if GET parameter cutted by framework (for example Codeigniter)
   * get parameter directly from url string
   * , added by KOLEX
   */
    protected function checkGetParam($paramname, $default = '') {
        $flag = false;
            ...
            if(is_array($url) && count($url)>1) {
                ...
                if(!empty($url[1])) {
                    ...
                    if(isset($params[$paramname])) {
                        $_GET[$paramname] = $params[$paramname];
                        $flag = true;
                    } else
                        $_GET[$paramname] = $default;
                }
            }
        }
		else
