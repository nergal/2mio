	...
	protected function build_where($rules,$relation=false){
		...
		if ($relation!==false) {
			if ($relation !== NULL && $relation !== 0) {
			    ...
			    array_push($sql,$this->config->relation_id["db_name"]." IS NULL");
			}
		}
...
//LogMaster::log('$data',$data);
//LogMaster::log('$request',$request);
//LogMaster::log('$this->config->text',$this->config->text);
...
/*! Implementation of DataWrapper for MySQLi
**/
...
//LogMaster::log('insert_id: ',$this->connection->insert_id);
//return $this->connection->insert_id;
		$insert_id = mysql_insert_id($this->connection);
		LogMaster::log('insert_id: ',$insert_id);	
		return $insert_id;
	}
