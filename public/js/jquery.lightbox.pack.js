	...
	$.fn.lightBox = function(settings) {
		...
		function _set_image_to_view() { // show the loading
			...
			objImagePreloader.onload = function() {
				...
				var iImg = new Image();
				iImg.src = settings.imageArray[settings.activeImage][0];
				
				if(iImg.width > $(window).width()) {
					iImg.width = $('#lightbox-image').get(0).width = $(window).width()-250;
				} else if(iImg.height > $(window).height()) {
					iImg.height = $('#lightbox-image').get(0).height = $(window).height()-250;
				}
				
				// Perfomance an effect in the image container resizing it
				_resize_container_image_box(iImg.width,iImg.height);
